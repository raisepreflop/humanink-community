#!/usr/bin/env python3
"""kdp_wrap.py (antes compose-kdp-wrap.py) — Build a KDP paperback wrap PANEL BY PANEL at exact
print geometry, instead of stretching a pre-baked design.

Vive en el motor OOXML (26-sep-2026) para que el Editor Studio lo lleve empaquetado con Pillow
(`ooxml wrap …`); la skill cover lo sigue llamando con skills/cover/scripts/compose-kdp-wrap.py,
que es un envoltorio de este módulo. Mismos argumentos en los dos sitios.
...
FONT_DIRS = ["/System/Library/Fonts/Supplemental", "/System/Library/Fonts",
             ...
             # Windows y Linux (el Studio instala este script en los dos; 26-sep-2026)
...
SANS        = font_path("Helvetica.ttc", "Arial.ttf", "arial.ttf", "HelveticaNeue.ttc", "DejaVuSans.ttf", "LiberationSans-Regular.ttf")
SANS_BOLD   = font_path("Helvetica.ttc", "Arial Bold.ttf", "arialbd.ttf", "Arial.ttf", "DejaVuSans-Bold.ttf", "LiberationSans-Bold.ttf")
SANS_ITAL   = font_path("Arial Italic.ttf", "ariali.ttf", "Helvetica.ttc", "DejaVuSans-Oblique.ttf")
if not (SANS or SANS_BOLD):
    sys.exit("✗ No encuentro ninguna fuente de sistema (Helvetica, Arial, DejaVu): no puedo componer el texto del lomo y la contraportada.")
...
def palette_for(bg, given=None):
    """Text colours that READ on the sampled background. The defaults were cream/gold for a black
    cover; on a light cover (a misty sky, a white page) the back would be unreadable. Keys the author
    gives win; the rest follow the background's luminance (26-sep-2026)."""
    lum = 0.2126*bg[0] + 0.7152*bg[1] + 0.0722*bg[2]
    dark_bg = lum < 140
    auto = ({"gold": "#D4AF37", "cream": "#F0E6D2", "body": "#E5E7EB", "muted": "#9CA3AF"} if dark_bg
            else {"gold": "#7A5C12", "cream": "#1F2937", "body": "#1F2937", "muted": "#4B5563"})
    return {**auto, **{k: v for k, v in (given or {}).items() if v}}
...
# ── spine ─────────────────────────────────────────────────────────────────
def draw_spine(canvas, g, title, author, gold, cream, direction="down"):
    ...
    # "down" (English): the spine reads TOP-TO-BOTTOM -> rotate clockwise (-90).
    # "up" (Spanish/continental): it reads BOTTOM-TO-TOP -> rotate counter-clockwise (+90). (26-sep-2026)
    strip = strip.rotate(90 if direction == "up" else -90, expand=True)
...
def draw_back(canvas, g, c, assets_dir):
    ...
    hay_logo = bool(c.get("logo")) and lp.is_file()
    logo_box = pt(64) if hay_logo else 0     # sin logo, la bio no se sangra (26-sep)
    tx = x0 + (logo_box + pt(10) if hay_logo else 0)
    bio_w = x1 - tx
    ...
    if hay_logo:   # sin logo, «assets_dir/» es la carpeta y no se abre (26-sep)
...
def main():
    ...
    c["palette"] = palette_for(bg, c.get("palette"))
    c["palette"]["bg"] = "#%02X%02X%02X" % bg
    c.setdefault("blocks", []); c.setdefault("bio", "")
    # The honest DPI rule (references/kdp-wrap.md): the front's real resolution on the printed panel.
    fw_px = Image.open(a.front).size[0]
    g["front_dpi"] = int(round(fw_px / (g["page_w_in"] + BLEED_IN)))
    ...
    if g["front_dpi"] < 300:
        print(f"  ⚠ The front image prints at ~{g['front_dpi']} DPI (< 300): the PDF is exact but the front will look soft.")
        print(f"    Regenerate it at ≥ {int(round((g['page_w_in'] + BLEED_IN) * 300))} px wide for 300 DPI.")
    print("="*64)
